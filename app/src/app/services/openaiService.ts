...
console.log("FUNCTION_BASE_URL", FUNCTION_BASE_URL);
...
export async function generateImage(
  ...
  try {
    ...
    const response = await fetch(`${FUNCTION_BASE_URL}/generate-image`, {
      method: "POST",
      headers: {
    ...
    return {
      success: false,
      error:
        data.error || "No image was generated. Please try a different prompt.",
    };
  } catch (error) {
...
export async function streamTextCompletion(
  ...
  try {
    const response = await fetch(`${FUNCTION_BASE_URL}/stream-completion`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
